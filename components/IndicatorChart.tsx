...
export default function IndicatorChart({
  ...
  useEffect(() => {
    ...
    if (histogram) for (const v of histogram) if (v !== null && v !== undefined) all.push(v);
    if (all.length === 0) return;

    const lo = min ?? Math.min(...all);
    const hi = max ?? Math.max(...all);
    const range = hi - lo || 1;
    const count = Math.max(...lines.map((l) => l.values.length), histogram?.length ?? 0);
  ...
  }, [lines, histogram, width, height, min, max, guides]);
