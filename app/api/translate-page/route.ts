 ...
 * page that completed. Returns one entry per element found.
 */
...
function parsePages(raw: string, expected: number): (PageItem[] | null)[] {
  const cleaned = raw
    .trim()
    .replace(/^```(?:json)?\s*/i, "")
    .replace(/```$/, "");
...
export async function POST(req: Request) {
    ...
    contents: [
      {
        ...
        parts: [
          ...
          { text: buildPrompt(sourceLang, targetLang, pages.length) },
        ],
      },
    ],
    generationConfig: {
  ...
  const results = parsePages(raw, pages.length);

  if (results.every((r) => r === null)) {
    console.error(
      "translate-page: unparseable response",
      "finish=",
