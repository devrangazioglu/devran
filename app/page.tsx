...
export default async function LandingPage() {
  const [{ t, locale }, quotes] = await Promise.all([getI18n(), highlights()]);
          ...
          <div className="container">
            <div className="ticker-strip">
              {quotes.map((quote) => (
                <Link
                  key={quote.id}
                  href={`/piyasa/${MARKETS[quote.market].slug}`}
          ...
          <div className="grid-4">
            {MARKET_IDS.map((id, index) => (
              <Reveal key={id} delay={index * 70}>
                <SpotlightCard>
                  <Link href={`/piyasa/${MARKETS[id].slug}`} className="market-card">
        ...
        <div className="container">
          <div className="footer-inner">
            ...
            <nav className="footer-links">
              {MARKET_IDS.map((id) => (
                <Link key={id} href={`/piyasa/${MARKETS[id].slug}`}>
